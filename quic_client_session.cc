...
ReliableQuicStream* QuicClientSession::kurt_CreateOutgoingDynamicStream() {
  if (!crypto_stream_->encryption_established()) {
    DVLOG(1) << "Encryption not active so no outgoing stream created.";
    return nullptr;
  }
  ...
  ReliableQuicStream* stream = CreateClientStream();
  ActivateStream(stream);
  return stream;
}


ReliableQuicStream* QuicClientSession::kurt_CreateClientStream() {
//  return new ReliableQuicStream(GetNextOutgoingStreamId(), this);
  return nullptr;
}
