...
/**
 * This is run from the above class, it inititases the session and tries to connect
 */
...
/**
 * the session is created and initialized here
 * every session also has an packet writer
 */
void QuicSimpleClient::StartConnect() {
  ...
  /*
   * The create session method takes a QuicConnection as argument.
   */
  ...
  /*
   * The actual session is located in quic_client_base
   */
  session()->Initialize();
...
void QuicSimpleClient::WaitForData(){
	while (WaitForEvents()) {}
}
